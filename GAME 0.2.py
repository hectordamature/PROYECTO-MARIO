...
def Play():
    global v,can,menu1I,menu2I,M,mar,E,soc,puntos,puntom,puntot
...
def salto2():
    global v,can,M,soc,sx,sy,est2,ya2,st2,ys2,est2
...
def muereS():
    global v,can,M,sx,sy,soc,est2,ya2,E,dif,h,ron,eron,vidaS
    ya2 = True
    soc.destroy()
    if(vidaS>0):
        vidaS-=1
...
def cambiarl():
    ...
    for x in range(len(E)):
        for y in range(len(E)):
            if(max(E[x][1],E[y][1])-min(E[x][1],E[y][1]) < 40 and E[x][2]==E[y][2] and E[x][0] != E[y][0]):
                if(E[x][0]<=1):
                    E[x][0]=2
                elif(E[x][0]== 3 or E[x][0]== 2):
                    E[x][0]=0
                if(E[y][0]<=1):
                    E[y][0]=2
                elif(E[y][0]== 3 or E[y][0]== 2):
                    E[y][0]=0
    v.after(50,cambiarl)
        ...
        if(E[x][0] <= 3):
            ...
            if(E[x][3]==1):
                if(E[x][2] + 65 >= my and E[x][2] + 30 <= my and E[x][1] <= mx + 30 and E[x][1] + 30 >= mx and ya == True):
                    E[x][0] = 5
                    pm += 10
                if(E[x][2] + 65 >= sy and E[x][2] + 30 <= sy and E[x][1] <= sx + 30 and E[x][1] + 30 >= sx and ya2 == True):
                    E[x][0] = 5
                    ps += 10
            if(E[x][0]==5):
                E[x][4].destroy()
...
def ronda():
    global ron,eron,v,can,menu1I,menu2I,M,mar,E,time,ya,ya2
    if(time>=difr):
        if(eron < 3+(ron*difc)):
            eron+=1
            ar = random.randint(1,2)
